...
function drawPixel() {
  if (cycleNum % 40 === 0 && cycleNum !== 0) {
    pixelImage += '\n';
    positionInRow = 0;
    //increment the positionToRow
  }
  //if positionInRow is xTotal + or - 1
  if (positionInRow === xTotal || positionInRow === xTotal - 1 || positionInRow === xTotal + 1) {
   // a new # needs to be added too pixelImage 
   pixelImage += '#';
   positionInRow++;
  }
   else {
    //a new . needs to be added to pixelImage
    pixelImage += '.';
    positionInRow++;
   }
}
